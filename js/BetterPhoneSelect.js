jQuery.fn.extend({
	BetterPhoneSelect : function(config) {
		 ...
		 */
				...
				$(this).addClass("myselect_options");
				
				//在下拉后面添加已选择项与选项框
				var select_obj = $(this)[0];
				var trim_selected =$(this)[0].options[$(this)[0].selectedIndex].text;
				trim_selected= trim_selected.replace(/\s+/g,"");
				$(this).after('<div class="selected_option" style="display:inline-block;border:1px solid #999;padding:2px 5px;background:white;" /* data="'+  +'"*/>'+
									trim_selected+
							  ...
							  '</div>'+
							  '<div class="all_select_options" style="display:none;position:fixed;background:rgba(0,0,0,.5);width: 100%;height: 100%;top: 0px;left: 0px;">'+
				...
				$(this).siblings('.all_select_options').find('.each_option').bind("click",function(){
					...
					//显示选中选项，插入下拉倒三角图标的插入
					$(this).parents('.all_select_options').siblings('.selected_option').text($(this).text().replace(/\s+/g,""))
					...
					/*
					$(this).parents('.all_select_options').siblings('.myselect_options').find('option').removeAttr('selected');
					//将隐藏的下拉选项与出现的下拉选项同步
					 
					$(this).parents('.all_select_options').siblings('.myselect_options').find('option:nth-child('+position_index+')').attr("selected","selected");
					*/
